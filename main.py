...
parser.add_argument('--experiment', type=str, choices=['permutated-mnist', 'svhn-mnist', 'mnist-svhn'],
...
parser.add_argument('--replay-mode', type=str, default='generative-replay',
...
if experiment == 'permutated-mnist':
    ...
    train_datasets = [get_dataset('mnist', permutation=p, capacity=capacity) for p in permutations]
    test_datasets = [get_dataset('mnist', train=False, permutation=p, capacity=capacity) for p in permutations]

    # decide what configuration to use.
    dataset_config = DATASET_CONFIGS['mnist']

elif experiment in ('svhn-mnist', 'mnist-svhn'):
    mnist_color_train = get_dataset('mnist-color', train=True, capacity=capacity)
    mnist_color_test = get_dataset('mnist-color', train=False, capacity=capacity)
...
label = '{experiment}-{replay_mode}-r{importance_of_new_task}'.format(
    ...
    importance_of_new_task=(1 if args.replay_mode == 'none' else args.importance_of_new_task)
)
...
if args.train:
    train(
        ...
        generator_c_updates_per_g_update=(args.generator_c_updates_per_g_update),
